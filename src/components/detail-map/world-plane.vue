<template>
  <div class="world-plane">
    <TheMap type="sea" :value="planeName" :info="positionInfo" :loading="loading" @on-search="fetchPositionInfoByCode" :render="renderInfoWindow" placeholder="请输入船名"/>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    // todo provide api
    fetchPositionInfoByCode (planeName) {
      if (this.loading) return
      this.loading = true
      truckLocation({ shipName: planeName })
        .then(res => {
