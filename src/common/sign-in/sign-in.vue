...
// import { AC_GLY } from '@/router/consoleAC'
export default {
  name: 'signIn',
  components: {
    // resign
  ...
  methods: {
    ...
    _userLogin () {
      if (this.checkVaild()) {
        ...
        // .then(res => {
        //   if (res.msg === 'success') {
        //     window.localStorage.setItem('dr_token', res.data.token)
        //     this.$message.success({
        //       message: '登录成功'
        //     })
        //     this.$store.commit('SET_ROLES', res.data.roles)
        //     window.localStorage.setItem('dr_roles', JSON.stringify(res.data.roles))
        //     if (res.data.roles.indexOf('AC_GLY') !== -1) {
        //       setTimeout(() => {
        //         this.$router.push({ path: '/console' })
        //       }, 0)
        //     } else if (res.data.roles.indexOf('AC_MH') !== -1) {
        //       setTimeout(() => {
        //         this.$router.push({ path: '/articleList' })
        //       }, 0)
        //     } else if (res.data.role === 'AC_ZKTGL') {
        //       setTimeout(() => {
  ...
  mounted () {
    // var db = openDatabase('anycase', '1.0', 'Test DB', 2 * 1024 * 1024)
    ...
    //     // console.log(this.mobile, 123)
    //   }, null)
    // })
    if (window.localStorage.getItem('dr_token') && window.localStorage.getItem('dr_roles')) {
      let drole = JSON.parse(window.localStorage.getItem('dr_roles'))
